...
DMARC, SPF, DKIM selectors, and BIMI straight from the authoritative nameserver — plus optional, disableable DNSSEC validation through a configurable DNS-over-HTTPS resolver, TLS-RPT, MTA-STS (including the policy file and MX coverage), and DANE/TLSA of the MX hosts. Missing records can be generated step by step under **Tools** (DMARC, SPF, TLS-RPT, MTA-STS, BIMI):
